...
use xilinxcorelib.mult_gen_v5_0_non_seq_comp.all;
use xilinxcorelib.mult_gen_v5_0_seq_comp.all;
...
architecture behavioral of mult_gen_v5_0 is

function check_loadb(mult_type, has_loadb : integer) return integer is
variable ret_val : integer;
begin
  if (has_loadb = 1 and mult_type > 2) then
     ret_val := 1;
  else
     ret_val := 0;
  end if;
...
begin
  ...
  check_inputs : process
   begin
      ...
      if ((c_has_a_signed = 1) or (c_a_type = c_signed)) and (c_a_width = 1) and (c_mult_type < 2) then
         assert false
            report "Cannot have a 1 bit signed value (A Port)" severity warning;
      end if;
      if (c_b_type = c_signed) and (c_b_width = 1) and (c_mult_type < 2) then
         assert false
  ...
  i1 : if (c_a_width = c_baat) and not(c_baat = c_a_width and c_b_value = "0" and c_mult_type = 2) generate
    mult_non_seq : mult_gen_v5_0_non_seq 
