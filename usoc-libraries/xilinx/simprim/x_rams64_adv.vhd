-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/rainier/VITAL/Attic/x_rams64_adv.vhd,v 1.2 2006/04/10 23:56:26 yanx Exp $
-------------------------------------------------------------------------------
...
begin
   ...
   rdwr_b_in <= RDWRB;
    ...
    d_out(7 downto 0) <=  abort_status when (abort_out_en = '1') else outbus;
    d_out_en <= d_o_en;
   ...
   rd_back_p : process ( cclk_in, rst_intl) begin
    if (rst_intl  = '0') then
        ...
        if (cmd_rd_flag  =  '1'  and  rdwr_b_in  =  '1'  and  csi_b_in  =  '0') then
               case (rd_reg_addr) is
               when "00000" => if (rd_data_cnt = 1)  then
               ...
               when "00111" => if (rd_data_cnt = 1)  then
                             outbus <= stat_reg(7 downto 0);
                          elsif (rd_data_cnt = 2) then
                             outbus <= stat_reg(15 downto 8);
                          elsif (rd_data_cnt = 3) then
                             outbus <= stat_reg(23 downto 16);
                          elsif (rd_data_cnt = 4) then
                             outbus <= stat_reg(31 downto 24);
                          end if;
               when "01100" => if (rd_data_cnt = 1) then
