 ...
 throw(ExOpen,std::bad_alloc)
{
 ...
 if (!SHSignalOn)
   {
    ...
    saio.sa_flags=0;
    saio.sa_flags=SA_RESTART;  // MMV: This is necesary for RASPBERRY since write is a SLOW process
    if (!CPPFIO_ComSerie_USE_THREAD)
 ...
 newtio.c_cflag= CS8 | CREAD | /*CSTOPB |*/ CLOCAL;
 ...
 //newtio.c_cc[VDSUSP]=_POSIX_VDISABLE;
        ...
        cfsetospeed(&newtio,B230400);            // 115200 baud
        cfsetispeed(&newtio,B230400);            // 115200 baud
 ...
 throw(ExWrite)
{
 ...
 if (DEBUG_ENVIA)
   {
//       printf("Enviando: %d bytes<",written);
    int i;
    for (i=0; i<l; i++)
       {
//        if (isprint(s[i]))
